...
approximation discussed in Revelt and Train [@ReveltTrain2000] and summarized
in upstream issue `jhelvy/logitr#38`:
...
panel_train <- subset(yogurt, id %in% 1:8 & obsID <= 150)
...
predict(panel_mxl, newdata = holdout, obsID = "obsID")
...
The same conditional workflow also applies to WTP-space mixed logit models.
For first-release honesty, confidence and prediction intervals are rejected in
conditional mode.
