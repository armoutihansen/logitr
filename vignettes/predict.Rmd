...
For panel mixed logit models, `predict()` continues to average over the unconditional population distribution. If you observe the same individuals again in new tasks, use `conditional_predict()` with an explicit conditioning panel instead. This reweights the model's stored simulation draws by how well each draw explains the individual's observed conditioning choices, following the logic in Revelt and Train [@ReveltTrain2000].

The example below uses a saved preference-space MXL model estimated on the `yogurt` data. We condition on prior observed choices for two households, then compare unconditional and conditional predictions for the same held-out choice tasks:

```{r}
mxl_pref <- readRDS(system.file("extdata", "mxl_pref.Rds", package = "logitr"))

conditioning_data <- subset(yogurt, id %in% c(1, 2) & obsID <= 10)
...
The two calls answer different questions: `predict()` treats those held-out tasks as new draws from the population distribution, while `conditional_predict()` uses the observed conditioning histories for the same households.
