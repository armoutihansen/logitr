...
Once a panel mixed logit model has been estimated, `predict()` can also be
used to generate **conditional** holdout probabilities for known individuals.
That workflow is documented in the prediction vignette and uses the
Revelt-and-Train style posterior reweighting of the fitted draws for each
individual's observed history.
